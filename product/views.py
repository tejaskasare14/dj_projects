...
# def index(request):
#    data={}
#    #Product.objects.all() this will fetch non active product also. so it is better to use filter
...
#    id_specific_cartitems=Cart.objects.filter(uid=user_id)
...
#    filterd_products=Product.objects.filter(q1 & q2)
...
#    q2 = Q(price__lte=max)
#    q3 = Q(is_active=True)
#    filterd_products=Product.objects.filter(q1 & q2 & q3)
#    data['products']=filterd_products
#    return render(request,'product/index.html',context=data)
...
def filter_by_category(request,category_value):
   ...
   global filterd_products
   if(category_value=="0"):
      filterd_products=Product.objects.filter(is_active=True)
      data['products']=filterd_products
   else:
...
def add_to_cart(request,pid):
   if request.user.is_authenticated:
      ...
      #cart=Cart.objects.create(pid=pid,uid=uid)
...
#       order=Order.objects.create(order_id=oid,quantity=data.quantity,pid=data.pid,uid=data.uid)
#       order.save()
#       cart.delete()
#    order=Order.objects.filter(uid=user_id)
#    total_price = 0
#    total_quantity=0
#    for item in order:
#       total_price=(total_price+item.pid.price)*(item.quantity)
